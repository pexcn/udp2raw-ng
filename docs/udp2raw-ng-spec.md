...
- 数据可能丢失、重复或乱序；
- 上层应用自行决定是否处理可靠性或顺序；
- 隧道层负责可选加密、强制认证、防重放、按业务触发的重连和恢复；空闲会话不发送保活流量。
...
- `--session-timeout-secs <N>`
  - client 在本地业务数据到达时，若超过该时间未收到可信 server 方向数据，则判定现有 session 可能失效并先重连；空闲时不发送 heartbeat 或自动重连。单位秒；默认建议 10 秒。
- `--reconnect`
...
- `Ready` 状态下不发送 heartbeat 或其他周期性保活帧；
- 空闲 session 保持 `Ready`，不因超时自行重连；本地业务数据到达时，若超过 `session-timeout-secs` 未收到可信 server 方向数据，则不得使用旧 session 发送该数据，必须先进入 `Reconnecting` 并发起新握手；
...
- client session timeout：10 s（仅用于本地业务到达时的认证回程活性判定）；
